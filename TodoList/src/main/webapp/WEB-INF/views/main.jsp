...
<%-- JSTL 라이브러리 현재 JSP에서 사용하겠다는 JSP 지시자 작성 --%>
...
<body>
   ...
   <form action="/todo/add"> <!-- 메서드 미작성 시 GET 기본 값  -->
       <h4> 할 일 추가 </h4>
       <div>
	...
	<table border="1" style="border-collapse : collapse;">
		...
		<tbody>
			<%-- c:forEach == JSTL의 향상된 for문
				 ...
				 var : 하나씩 꺼내서 저장할 변수
				 varStatus : 반복 상태 관력 객체(몇 번 인덱스, 첫바퀴? 마지막?)
			 --%>
	...
	<%-- JSP에서 Java(EL, JSTL), Front(HTML, CSS, JS)중
		 Java가 우선 순위가 높기 때문에 스크립트 태그 안에
		 EL사용해서 자바 코드 사용 가능
	 --%>
	 <!-- session scope에 message 속성이 있으면 밑에 JSTL 실행 -->
	<c:if test="${not empty sessionScope.message}">
		<script>
			// EL을 이용해서 String을 출력할 때 꼭 "" 추가하기!!
